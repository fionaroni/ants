...
# Name: Fiona Tang, Patrick Leng
# Email: fionatagious@gmail.com
...
class Place:
    ...
    def __init__(self, name, exit=None):
        ...
        if not self.exit == None: # if A's entrance is B
            exit.entrance = self
    ...
    def add_insect(self, insect):
        ...
        if insect.is_ant():
            ...
            if self.container == True: # ant occupying place is a container
                self.ant = insect
                ant.contain_ant(insect)
            elif insect.container == True: # insect is the container
                self.insect = insect
                insect.contain_ant(ant)
            else: # if neither ant is a container
...
class Ant(Insect):
    ...
    def __init__(self, armor=1): # understand usage of this default argument, alternatives?
        """Create an Ant with an armor quantity."""
        Insect.__init__(self, armor)
        # why not write self.armor = 1 here (instead of armor=1) in param?
        # is it because __init__(self, armor=1) is bound to Insect.__init(self, armor)?

    def is_ant(self):
        return True

    def can_contain(self, other):
        if self.container == True and self.contain_ant(other) == None and other.container == False: 
            return True
        else:
            return False
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        while not current_place == hive: # ask about how hive works
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        copy_bees = copy.copy(self.place.bees)
        self.armor -= amount
        if self.armor <= 0:
...
class WallAnt(Ant):
    ...
    def __init__(self):
        Ant.__init__(self)
        self.armor = 4 # overrides armor = 1 in default param of Ant constructor
...
class HungryAnt(Ant):
    ...
    def action(self, colony):
        ...
        if self.digesting == 0: # is not digesting, ready to eat a bee!
            self.eat_bee(bee)
        else: # is digesting, need to wait before eating a bee!
            self.digesting -= 1
